---
title             : "I Regulate therefore I am: Investigating Congruency-effects in Autonomic Responses and their Role in Interoceptive Processing "
shorttitle        : "I Regulate therefore I am"
author: 
  - name          : "Lilla Hodossy"
    affiliation   : "1"
...
affiliation:
  ...
  - id            : "2"
    institution   : "The Warburg Institute, School of Advanced Studies"
authornote: |
...
```
```{r, echo=FALSE,fig.cap = "Effect of Biofeedback Congruency (Congruent vs Incongruent) on the square root transformed HF-HRV (nu) values. The raincloud plot provides data distribution, the central tendency by boxplots and the jittered presentation of our raw data. Error bars indicate 95% confidence intervals around the estimates of the linear mixed effects model. The random intercept model depicting this lower level congruency effect also includes baseline HF-HRV (nu) as a covariate."}
low_abs
```

### 2.2.1 Discussion of Experiment 1.
We observed changes in HF-HRV associated with the integration of exteroceptive and interoceptive signals on a lower sensory level, but the level of task involvement did not have an additional effect. Receiving incongruent visual feedback with one's own cardiac activity was associated with a lower level of HF-HRV when compared to congruent feedback. These results indicate that changes in HF-HRV can potentially serve as an index of PE-s as it is sensitive to multisensory congruencies.
...
```{r, echo=FALSE, fig.cap= "Higher-level congruency effect on the changes in HF-HRV (nu) from baseline depicted as (A) an interaction between Biofeedback Congruency (biofeedback belongs to Congruent vs Incongruent) and Belief (Other vs Self) and as a (B) main effect where higher-level Congruency is coded as a single predictor (Congruent with Belief vs Incongruent with Belief). The raincloud plot provides data distribution, the central tendency by boxplots and the jittered presentation of our raw data. Error bars indicate 95% confidence intervals around the estimates of the linear mixed effects model. Both random intercept models include baseline HF-HRV (nu) as a covariate."}
